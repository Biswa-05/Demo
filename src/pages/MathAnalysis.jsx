...
async function getAIResponse(messages) {
  try {
    ...
    const res = await fetch(endpoint, {
      method: "POST",
      headers: {
        Authorization: `Bearer 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`,
...
export default function MathAnalysis() {
  // Chatbot state and logic with sessionStorage persistence
  const defaultWelcome = {
    role: "assistant",
    content:
      "Hi! Ask me anything about Kolam mathematical patterns, symmetry, or regeneration.",
  };
  ...
  // Restart chat: clear history and sessionStorage
  const handleRestart = () => {
    const welcome = {
      role: "assistant",
      content:
  ...
  // Close chat: hide chat UI
  const handleClose = () => {
    setChatOpen(false);
  };

  // Reopen chat: show chat UI
  const handleOpen = () => {
    setChatOpen(true);
  };
  ...
  return (
    <section className="relative py-16 px-6 bg-gradient-to-br from-orange-50 via-white to-yellow-50 overflow-hidden">
      ...
      <motion.div
        ...
        className="absolute top-0 left-0 w-64 h-64 bg-orange-300 rounded-full blur-3xl"
      ></motion.div>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.15 }}
        transition={{ duration: 2 }}
        className="absolute bottom-0 right-0 w-72 h-72 bg-yellow-400 rounded-full blur-3xl"
      ></motion.div>

      {/* Title with Animation */}
      <motion.h2
        className="relative z-10 text-4xl md:text-5xl font-extrabold text-gray-900 mb-10 text-center flex items-center justify-center gap-3"
      ...
      {/* AI Chat Bot Section - added below */}
      {chatOpen ? (
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.9, ease: "easeOut" }}
          className="relative mt-20 z-10 flex flex-col items-center"
        >
          <motion.div
            className="rounded-2xl shadow-2xl bg-gradient-to-br from-orange-100 via-white to-yellow-50 w-full max-w-3xl mx-auto p-7 border-2 border-orange-200"
            initial={{ scale: 0.95 }}
            animate={{ scale: 1 }}
          ...
          >
            <div className="flex items-center gap-2 mb-5 justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-2xl md:text-3xl text-orange-800 font-serif">
                  AI Kolam Math Chat
                </span>
              </div>
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1 rounded-lg bg-orange-200 text-orange-900 font-semibold hover:bg-orange-300 transition-all text-sm"
                  title="Restart chat"
                >
                  Restart
                </button>
                <button
                  onClick={handleClose}
                  className="px-3 py-1 rounded-lg bg-yellow-200 text-yellow-900 font-semibold hover:bg-yellow-300 transition-all text-sm"
                  title="Close chat"
                >
                  Close
                </button>
            ...
            <div className="overflow-y-auto max-h-80 mb-3 px-1 hide-scrollbar transition-all">
              {messages.map((msg, idx) => (
                ...
                >
                  <div
                    className={`py-2 px-4 rounded-2xl text-base font-mono font-semibold ${
                      msg.role === "user"
                        ? "bg-orange-200 text-gray-900"
                        : "bg-yellow-100 text-orange-800 shadow-md"
                    }`}
                  >
                    {msg.content}
                  </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleSend}
                disabled={loading}
                className="rounded-full bg-gradient-to-r from-orange-400 to-yellow-300 p-3 hover:scale-105 hover:from-orange-500 transition-all"
              >
                <Send size={23} className="text-white" />
              </button>
              {loading && (
                <motion.span
                  className="ml-2 text-orange-700 animate-pulse font-semibold"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
        ...
        <div className="flex justify-center mt-20">
          <button
            onClick={handleOpen}
            className="px-6 py-2 rounded-xl bg-orange-300 text-orange-900 font-bold text-lg shadow hover:bg-orange-400 transition-all"
          >
            Open AI Chat
          </button>
        </div>
